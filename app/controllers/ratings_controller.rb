class RatingsController < ApplicationController
  ...
  def index
    ...
    @rhetoricchart = LazyHighCharts::HighChart.new('pie') do |f|
      f.chart({ :defaultSeriesType=>"pie", :width => 250, :height  => 300, :marginRight => 70 } )
    ...
    @reusechart = LazyHighCharts::HighChart.new('chart') do |f|
      ...
      f.options[:title][:text] = nil
      f.options[:plotOptions] = { :column => { :pointWidth => 60, :borderWidth => 0 } }
    ...
    @activitychart = LazyHighCharts::HighChart.new('chart') do |f|
      ...
      f.series(:name => "Ratings", :color => "#95ab63", :data => days.map {|a| [Date.strptime(a[0], '%Y-%m-%d').to_time.in_time_zone.to_i * 1000, a[1]] })
